(function () {
  ...
  function buildSVG() {
    ...
    [`M 30 ${BELL_BOT_Y} Q 18 38 12 18`, `M 30 ${BELL_BOT_Y} Q 30 36 30 ${TIP_Y + 2}`, `M 30 ${BELL_BOT_Y} Q 42 38 48 18`].forEach((d) =>
      bellGroup.appendChild(
        el("path", {
          d,
  ...
  const tentacleEls = Array.from({ length: TENTACLES.length }, (_, i) => svg.getElementById(`jft-${i}`));
  ...
  function animateTentacles(t) {
    TENTACLES.forEach(({ x, len }, i) => {
      ...
      tentacleEls[i].setAttribute("d", `M ${x} ${y0} Q ${x + w1} ${y1} ${x} ${(y0 + y1) / 2 + 4} Q ${x + w2} ${y2} ${x} ${y3}`);
    });
  }
